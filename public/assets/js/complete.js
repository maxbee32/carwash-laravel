...
async function checkStatus() {
  ...
  // If payment is successful, trigger backend request to save booking data
  if (paymentIntent.status === "succeeded") {
    await saveBookingAfterPayment(paymentIntent.id);  // Call saveBookingAfterPayment with the payment intent ID
  }
}
...
async function saveBookingAfterPayment(paymentIntentId) {
  ...
  const response = await fetch(bookingSaveUrl, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      payment_intent_id: paymentIntentId,  // Pass the payment intent ID to the backend
    })
  });
